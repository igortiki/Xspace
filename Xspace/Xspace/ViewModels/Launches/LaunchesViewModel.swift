...
final class LaunchesViewModel: ObservableObject {
    ...
    @Published var cellViewModels: [LaunchCellViewModel] = []
    ...
    // MARK: - Computed
    var launchesSectionTitle: String { "Launches" }
    var emptyLaunchesText: String { "No launches found." }
    var retryButtonText: String { "Retry" }
    ...
    func cellViewModel(at index: Int) -> LaunchCellViewModel {
    ...
    // MARK: - Fetch Logic
    private func fetchPage(reset: Bool, filters: LaunchFiltersModel) async {
        ...
        do {
            ...
            let enriched = launchResponse.docs.map {
                EnrichedLaunch(launch: $0, rocket: rocketMap[$0.rocket])
            }
    ...
    private func fetchLaunchesPage(reset: Bool, filters: LaunchFiltersModel) async throws -> LaunchesResponse {
        if reset {
            currentPage = 1
            hasNextPage = true
            cellViewModels = []
